...
Apply a **free-first rule** to every feature: prefer browser-native Web APIs, local computation, open-source packages, and existing free-tier services before proposing any paid provider. Do not add a paid API, paid SDK, subscription service, or usage-billed integration when a reliable free alternative can satisfy the MVP requirement. If a paid provider is ever genuinely necessary, explain the cost and ask for approval first. Unofficial free-provider adapters must remain server-only, isolated behind a replaceable feature boundary, treated as best-effort infrastructure, and paired with a graceful fallback; never expose provider credentials or make the UI depend on undocumented provider guarantees.
