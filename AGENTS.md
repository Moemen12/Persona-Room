...
Apply a **free-first rule** to every feature: prefer browser-native Web APIs, local computation, open-source packages, and existing free-tier services before proposing any paid provider. Do not add a paid API, paid SDK, subscription service, or usage-billed integration when a reliable free alternative can satisfy the MVP requirement. If a paid provider is ever genuinely necessary, explain the cost and ask for approval first. For voice, prefer local Kokoro neural synthesis with device-appropriate WebGPU/WASM execution; use remote Edge Neural synthesis only as a multilingual or local-model fallback. Large browser models must load in a Web Worker, never block the React main thread, never download on initial page render, respect `saveData`/slow-network signals, and expose a clear warm-up state. Unofficial free-provider adapters must remain server-only, isolated behind a replaceable feature boundary, treated as best-effort infrastructure, and paired with a graceful fallback; never expose provider credentials or make the UI depend on undocumented provider guarantees.
